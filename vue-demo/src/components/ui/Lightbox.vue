<template>
  <div>
    ...
    <gallery v-if="!isIE10Mode" ref="gallery" :images="vueGalleryImageLinks" :index="vueGalleryIndex" @close="vueGalleryIndex = null" :options="vueGalleryOptions"></gallery>

    <!-- IE10 fallback -->
    <b-modal v-if="isIE10Mode" :visible="vueGalleryFallback" @hidden="vueGalleryIndex=null" size="xl" hide-footer hide-header content-class="bg-transparent" body-class="p-0">
      <button class="close position-absolute text-white opacity-100 px-3" style="font-size:40px;right:0" @click="vueGalleryIndex=null">&times;</button>
      <img v-if="vueGalleryIndex!==null" :src="vueGalleryImageLinks[vueGalleryIndex]" class="mw-100 rounded">
    </b-modal>
    ...
    <hr class="container-m-nx border-light mt-5 mb-4">
    ...
    <div class="row">
      <div class="col-md-3" v-for="(image, i) in vImgImages" :key="i">
        <a class="img-thumbnail" href="javascript:void(0)">
          <img v-img="vImgOptions" :src="publicUrl + image.url" :alt="image.altText" class="img-fluid">
        </a>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    vueGalleryImageLinks () {
      return this.vueGalleryImages.map(img => this.publicUrl + img.url)
    },

    vueGalleryFallback () {
      return this.vueGalleryIndex !== null
    }
  }
}
</script>
