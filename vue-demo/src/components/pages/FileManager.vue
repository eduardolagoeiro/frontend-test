  ...
  <div>
    ...
    <div class="file-manager-container" :class="`file-manager-${viewMode}-view`">
      ...
      <div class="file-item" v-for="item in list" :key="item.name" :class="{'selected border-primary': selected.includes(item), 'focused': focused === item || dropdownOpened === item}" @focusin="focusIn(item)" @focusout="focusOut()">
        ...
        <div class="file-item-img" :style="{'background-image': `url(${publicUrl}${item.path})`}" v-if="isImage(item)"></div>
        ...
        <b-dropdown class="file-item-actions" variant="default rounded-pill icon-btn borderless md-btn-flat hide-arrow" size="sm" :right="!isRTL" @shown="dropdownShown(item)" @hidden="dropdownHidden()">
